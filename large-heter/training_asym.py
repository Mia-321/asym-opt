...
def RunExp(args, dataset, data, Net, percls_trn, val_lb):
    ...
    name_w = 'lin'
    mt_theta, vt_theta, mt_w, vt_w, mt_theta_grad_norm, mt_w_grad_norm, mt_theta_value_norm, mt_w_value_norm = init_custom_optimizer(
        model, name_w, device)

    time_run=[]
    for epoch in range(args.epochs):
        t_st = time.time()
        mt_theta, vt_theta, mt_w, vt_w, para_theta_his, para_w_his, mt_theta_grad_norm, mt_w_grad_norm, mt_theta_value_norm, mt_w_value_norm = train(
            model, optimizer, data, args, name_w, epoch,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, choices=['Cora','Citeseer','Pubmed','Chameleon','Squirrel','Actor','Texas','Cornell', 'Wisconsin',
                                                        'amazon_ratings', 'minesweeper', 'questions',  'roman_empire',  'tolokers'],default='Cora')
