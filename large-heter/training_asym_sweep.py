...
def train(model, optimizer, data, args, name_w, epoch, mt_theta, vt_theta, mt_w, vt_w,
    ...
    loss.backward()
    if args.asym:
        mt_theta, vt_theta, mt_w, vt_w, para_theta_his, para_w_his, mt_theta_grad_norm, mt_w_grad_norm, mt_theta_value_norm, mt_w_value_norm = update_model(model, name_w, mt_theta, vt_theta,
...
def RunExp(args, dataset, data, Net, percls_trn, val_lb):
    ...
    if args.net == 'ChebNetII' and args.dataset in ['Chameleon', 'Squirrel']:
        Net = ChebNetII_V
    ...
    if args.semi_rnd:
        ...
        elif args.dataset in ['amazon_ratings', 'minesweeper', 'questions', 'roman_empire', 'tolokers']:
    ...
    name_w = 'lin'
    mt_theta, vt_theta, mt_w, vt_w, mt_theta_grad_norm, mt_w_grad_norm, mt_theta_value_norm, mt_w_value_norm = init_custom_optimizer(model, name_w, device)

    time_run = []
    for epoch in range(args.epochs):
        t_st = time.time()
        mt_theta, vt_theta, mt_w, vt_w, para_theta_his, para_w_his, mt_theta_grad_norm, mt_w_grad_norm, mt_theta_value_norm, mt_w_value_norm = train(model, optimizer, data, args, name_w, epoch,
                    mt_theta, vt_theta, mt_w, vt_w,
                    para_theta_his, para_w_his, mt_theta_grad_norm, mt_w_grad_norm, mt_theta_value_norm, mt_w_value_norm)

        time_epoch = time.time() - t_st  # each epoch train times
        time_run.append(time_epoch)
        [train_acc, val_acc, tmp_test_acc], preds, [train_loss, val_loss, tmp_test_loss] = test(model, data, metric_fn)
...
def exp_sweep(args, dataset, data, Net, percls_trn, val_lb):



    project_name = 'heter-' + args.net + '-' + args.base + '-asym-' + str(args.asym)
    with wandb.init(project=project_name, config = wandb.config) as run:
        config = wandb.config
        print(f'wandb config:{config}')
        run_name = ''
        for key, value in config.items():
            run_name = run_name + key + '-' + str(value)

        run.name = args.dataset + "-" + args.base + run_name

        for key_item, _ in vars(args).items():
            # print(f'keys:{key_item}')
            if key_item in config:
                # print(f'--{key_item}')
                setattr(args, key_item, config[key_item])

        if args.net == 'PolyNet':
            args.lr1 = args.lr2 = args.lr3 = args.lr
            args.wd1 = args.wd2 = args.wd3 = args.weight_decay
        ...
        for RP in tqdm(range(args.runs)):
            ...
            best_step, best_train_acc, best_train_loss, best_test_acc, best_test_loss, best_val_acc, best_val_loss, theta, time_run = RunExp(
        ...
        for i in range(args.runs):
            ...
            run.log({"run_exp": i, "test acc": test_acc_list[i], "best_step": best_step_list[i]})
        ...
        mean_gap_acc, std_gap_acc = np.mean(gap_acc_tensor), np.max(np.abs(
            sns.utils.ci(sns.algorithms.bootstrap(gap_acc_tensor, func=np.mean, n_boot=1000),
                         95) - gap_acc_tensor.mean()))
        ...
        # wandb log
        run.log({"test_acc_mean": test_acc_mean})
        run.log({"test_acc_std": test_acc_std})
        run.log({"train_acc_mean": train_acc_mean})
        run.log({"train_acc_std": train_acc_std})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str,
                        choices=['Cora', 'amazon_ratings', 'minesweeper', 'questions',  'roman_empire',  'tolokers'], default='Cora')
    parser.add_argument('--device', type=int, default=0, help='GPU device.')
    parser.add_argument('--runs', type=int, default=10, help='number of runs.')
    parser.add_argument('--net', type=str,
                        choices=['GCN', 'APPNP', 'ChebNet', 'MLP', 'ChebNetII', 'ChebBase', 'GPRGNN', 'BernNet',
                                 'PolyNet'], default='ChebNetII')
    ...
    parser.add_argument('--beta_1_theta', type=float, default=0.9, help='beta_1.')
    parser.add_argument('--beta_2_theta', type=float, default=0.999, help='beta_2.')
    ...
    parser.add_argument('--opt_step', type=int, default=5, help='optimizer step.')
    ...
    config_file_name = args.net + '-' + args.base + '.yaml'
    config_file_path = os.path.join('./configs/', config_file_name)
    project_name = 'heter-' + args.net + '-' + args.base + '-asym-' + str(args.asym)
    sweep_name = 'heter-' + args.net + '-' + args.base + '-asym-' + str(args.asym) + '-' + args.dataset

    print(f'config file path:{config_file_path}')
    with open(config_file_path) as f:
        sweep_configuration = yaml.load(f, Loader=yaml.FullLoader)
        sweep_configuration['name'] = sweep_name
        print(f'open config file and sweep conf:{sweep_configuration}')
        sweep_id = wandb.sweep(sweep=sweep_configuration, project=project_name)
        # wandb_train_func = functools.partial(my_train_func, x)
        wandb.agent(sweep_id, function= functools.partial(exp_sweep, args, dataset, data, Net, percls_trn, val_lb) )  # , count=3)
        # wandb.agent(sweep_id, function= exp_sweep(args, dataset, data, Net, percls_trn, val_lb) )  # , count=3)
