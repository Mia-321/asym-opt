...
from torch_geometric.datasets import Planetoid, Amazon, Flickr, Coauthor, WikipediaNetwork, Actor
...
def DataLoader(name):
    ...
    if name in ['cora', 'citeseer', 'pubmed']:
        dataset = Planetoid(osp.join(root_dir, name), name, transform=T.NormalizeFeatures())
    elif name in ['chameleon', 'actor', 'squirrel']:
        dataset = Chame_Squir_Actor(root=root_dir, name=name, transform=T.NormalizeFeatures())
    elif name in ['texas', 'cornell', 'wisconsin']:
        dataset = WebKB(root=root_dir,name=name, transform=T.NormalizeFeatures())
    elif name in ['computers', 'photo']:
        if name == 'photo':
            dataset = Amazon(root=root_dir, name='Photo', transform=T.NormalizeFeatures())
        elif name == 'computers':
            dataset = Amazon(root=root_dir, name='Computers', transform=T.NormalizeFeatures())
    elif name in ['flickr']:
